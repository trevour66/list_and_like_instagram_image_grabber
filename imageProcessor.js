...
const imageProcessor = async () => {
    try {
        ...
        if (files.length === 0) {
            logger.error('Unable to scan directory:', {timestamp: new Date().toLocaleString()})
        ...
        for (let index = 0; index < files.length; index++) {
            ...
            if (match) {
                ...
                fs.unlink(filePath, (err) => {
                    if (err) {
                        logger.error(`Error deleting file ${file}: ${err})`, {timestamp: new Date().toLocaleString()})
        ...
        logger.error(`Error | imageProcessor | ${error.message}`, {timestamp: new Date().toLocaleString()})
...
const saveFiles = async () => {
    ...
    for (let index = 0; index < files.length; index++) {
        ...
        await uploadFileToGCS(filePath, destination)
            ...
            .catch((err) => {
                logger.error(`Error uploading file: ${err}`, {timestamp: new Date().toLocaleString()})
        ...
        // Further processing with the post_id can be done here
            ...
            logger.error(`File: ${file} does not match the expected pattern.`, {timestamp: new Date().toLocaleString()})
